...
case object Menu {
    ...
    def searchOptions: Unit = {
      ...
      if(opt == 1) {
        ...
        option_2 = initialOptions; // Reads the option, limits the possibilities, valid ints only
        println("Any specific manufacturer brand? ('0' for none)");
        var opt_3 = scala.io.StdIn.readLine();

        if(option_2 == 2) {
          println("Any specific model name? ('0' for none)");
          opt_4 = scala.io.StdIn.readLine();
        }

        // Adding a new option to the car model option.
        ...
        else if(option_2 == 2 && opt_3 == "0" && opt_4 == "0") 
        ...
        else if(option_2 == 2 && opt_4 != "0")
          out = collection.aggregate( Seq(filter(equal("model_name", opt_4)),group("$model_name", push("manu_n", "$manufacturer_name"), sum("num",1), avg("avg_p", "$price_usd")), sort(ascending("avg_p")))).results;        
        else if(option_2 == 2 && opt_4 == "0")
          out = collection.aggregate(Seq(filter(equal("manufacturer_name", opt_3)), group("$model_name", push("manu_n", "$manufacturer_name"), sum("num", 1), avg("avg_p", "$price_usd")), sort(ascending("avg_p")))).results;
      }
      ...
      // Adding an option to print out all the fields on one car
      if(opt == 1) {
        println("\nIf you would like more information on a certain car, please enter the ID. If not enter '0'");
        var moreDetails = scala.io.StdIn.readInt();
        if(moreDetails != 0){
          println(s"Looking for the ID: $moreDetails");
          collection.find(equal("_id", moreDetails)).projection(excludeId()).first().printHeadResult(); // Only one document since _id are unique          
        }
      }
    }
